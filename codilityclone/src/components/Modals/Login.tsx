...
const Login: React.FC<LoginProps> = () => {
    ...
    const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
        console.log(inputs)
        e.preventDefault()
        if (!inputs.email || !inputs.password) return alert('Please fill all required fields')

        try {
            const loginUser = await signInWithEmailAndPassword(inputs.email, inputs.password)
            if (!loginUser) return
            router.push('/')
        } catch (error: any) {
            alert(error.message)
